"""
Disease-specific explanations and diet recommendations (EN + KN).
Lightweight lookup only – no model loading. Use this instead of bert_nlg for fast response.
"""
from typing import Tuple

# English explanations (one per disease) – ready to display
EXPLANATIONS_EN = {
    "Atelectasis": "This X-ray shows areas of partial collapse in part of the lung, a condition called atelectasis. When part of the lung is not fully inflated, it can reduce oxygen levels and cause breathing difficulty.",
    "Brain_Tumor": "This scan suggests an abnormal tissue growth inside the brain, which may represent a brain tumour. Tumours can press on normal brain tissue and may cause headaches, seizures, or weakness.",
    "Cardiomegaly": "The heart silhouette appears noticeably larger than normal, a finding called cardiomegaly. An enlarged heart can occur when the heart muscle has to work harder over time.",
    "Consolidation": "Parts of the lung appear more dense and white, a sign of consolidation. This often happens when the air spaces fill with fluid, pus, or cells during infection or inflammation.",
    "Edema": "There is a pattern suggesting extra fluid in or around the lungs, called pulmonary edema. This can be related to heart problems or other conditions that cause fluid overload.",
    "Effusion": "The image shows extra fluid collecting around the lungs, known as a pleural effusion. This fluid can compress the lung and make breathing uncomfortable.",
    "Emphysema": "The lungs appear markedly over-inflated with areas of tissue damage, consistent with emphysema. This long-term damage reduces the lung's ability to move oxygen into the blood.",
    "Fibrosis": "There are streaky, fibrotic scar-like changes in the lung tissue, suggesting pulmonary fibrosis. Scarring makes the lungs stiff and can lead to chronic breathlessness.",
    "Hernia": "The image suggests that abdominal structures may be protruding into the chest cavity, consistent with a hernia. This can change normal organ position and sometimes affect breathing.",
    "Infiltration": "The lungs show patchy white areas called infiltrates. These can be caused by infections, inflammation, or other lung conditions.",
    "Mass": "There is a distinct rounded shadow in the image, which may represent a mass or growth. Further tests are usually needed to know its exact nature.",
    "No_Brain_Finding": "This scan does not show a clear abnormal focal lesion in the brain. However, imaging does not replace a full clinical examination by a neurologist.",
    "No_Lung_Finding": "On this X-ray, there is no obvious significant abnormality seen in the lungs. Even when images look normal, symptoms should still be discussed with a doctor.",
    "Nodule": "A small round spot is visible in the lung called a nodule. Most nodules are harmless, but some need follow-up imaging to be sure.",
    "Pleural": "The lining around the lungs, called the pleura, looks somewhat thickened or irregular. Pleural changes can be due to past infection, inflammation, or other disease.",
    "Pneumonia": "This X-ray shows cloudy areas in part of the lung that suggest pneumonia, an infection of the air spaces. Pneumonia can cause fever, cough, and breathing difficulty.",
    "Pneumothorax": "The image suggests free air has leaked into the space around the lung, called a pneumothorax (collapsed lung). This can cause sudden chest pain and shortness of breath.",
    "Tuberculosis": "This scan shows characteristic changes in the lungs that are consistent with tuberculosis (TB). TB is a long-term infection that usually needs several months of medicine.",
}
...
def get_explanation_and_diet(disease_name: str) -> Tuple[str, str, str, str]:
    """
    Return (explanation_en, explanation_kn, diet_en, diet_kn) for the given disease.
    Instant lookup – no model loading. Unknown diseases get generic fallback text.
    """
    dn = disease_name.replace("_", " ")

    expl_en = EXPLANATIONS_EN.get(
        disease_name,
        f"This image shows patterns that may indicate {dn}. Please consult a specialist doctor for confirmation.",
    ) + EXTRA_EXPL_EN

    diet_en = DIETS_EN.get(
        disease_name,
